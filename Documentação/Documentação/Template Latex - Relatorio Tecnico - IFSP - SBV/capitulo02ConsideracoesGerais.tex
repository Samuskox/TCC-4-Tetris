...
\section{Tipos de Jogos não Digitais} %será que eu mudo pra TIpos de jogos Tradicionais??
	...
	permitindo o blefe, por exemplo o poker \cite{lucchese2009conceituaccao}. Abaixo está a imagem de um baralho clássico que está presente em vários jogos de cartas:
...
\begin{figure}[!htbp]
	...
	\\\textbf{Fonte:} https://segredosdomundo.r7.com/dicas-de-jogos-de-baralho/
\end{figure}
\FloatBarrier
	...
	ter mais pontos ou valores que outros jogadores \cite{lucchese2009conceituaccao}. Um exemplo de um jogo de tabuleiro, o xadrez abaixo:
...
\FloatBarrier
	...
	física é que defini para um jogador ter a habilidade de jogar um jogo atlético. Abaixo está um exemplo de um jogo atlético, o basquete:
...
\FloatBarrier
	...
	distâncias \cite{UltimateVideoGames}. Um clássico dos jogos de FPS é mostrado abaixo:
...
Há alguns jogos que utilizam planos paralelos no plano 2D para movimentos do jogador \cite{codakid2025_video_game_genres, UltimateVideoGames}. Um bom exemplo de um jogo de luta chamado "\textit{Street Figher}"é mostrado abaixo
...
se tornou um subgenêro muito famoso \cite{UltimateVideoGames,codakid2025_video_game_genres}. Um exemplo de jogo de plataforma é mostrado abaixo:
...
\begin{figure}[!htbp]
	...
	\includegraphics[scale=0.4]{imagens/JogosPlataformas.jpg}
	\\\textbf{Fonte:} Elaborada pelo autor
\end{figure}
\FloatBarrier
...
Um bom exemplo de jogos de aventura é a franquia de jogos "\textit{The legend of Zelda}", abaixo é mostrado a imagem de um dos jogos dessa franquia:

\FloatBarrier
\begin{figure}[!htbp]
	\centering
	\caption{Jogo de Aventura \textit{The Legend of Zelda: Ocarina Of Time}}
	\includegraphics[scale=0.4]{imagens/JogosAventura.JPG}
	\\\textbf{Fonte:} Elaborada pelo autor
	\label{fig:exemplo}
\end{figure}
\FloatBarrier
...
e por último renderiza o jogo no estado atual e assim é feito a repetição desse processo \cite{Alves2020}, como mostrado na imagem abaixo:
...
Existem muitos tipo de nós disponíveis, tipos de nós para gráficos, física, interface de usuário, audio e de outros tipos, esses 
nós são organizados em uma cena de acordo como o desenvolvedor quiser para comportamentos mais complexos \cite{salmela2022_godot, godot_docs}.
Assim como um script pode ser anexado á um nó para também conseguir um comportamento específico. \textit{Godot Engine} tem uma linguagem para programar
...
Um exemplo do ambiente de desenvolvimento na \textit{Godot Engine} é mostrado abaixo:
...
já na renderização em 2D também oferece luz, sombra e máscaras 2D. 
...
é possível encapsular dados dentro de um bloco e criar métodos demanipulação desses dados. Um exemplo de encapsulamento é mostrado na imagem abaixo: 
...
Esses arquivos estão em um formato chamado bytecode e assim podem ser executados em qualquer interpretador de Java, o código precisa ser compilado 
apenas uma vez, pois os bytecodes são gerados dentro do arquivo classe e podem ser executados em qualquer plataforma de hardware e software \cite{indrusiak_java, deitel2016java}.
...
Com esses módulos disponíveis na biblioteca LibGDX, abaixo mostra um diagrama como esses módulos são usados em um ciclo de vida de um jogo:
...
manipulação de \textit{bitmaps} e editor de texturas, junto com isso \cite{libgdx_wiki, firmansyahRPGLibgdx}, existem métodos principais, são essenciais para manter o ciclo de vida de um jogo, são eles:

\begin{table}[h]
\centering
\caption{Ciclo de Métodos do LibGDX (Adaptado de \cite{firmansyahRPGLibgdx})}
...
C++ é uma linguagem que mistura tanto a programação orientada a objeto tanto quanto a programação procecedural, assim o programador tem uma
...
Os códigos feitos em C++ ao serem executados passam por 6 fases: edição, pré-processamento, compilação, linkagem, carregamento e execução, como mostrado abaixo:
...
Assim como a LibGDX e o godot, a Raylib tem funções próprias que compõe a arquitetura de um jogo \cite{raylib2025}, abaixo na tabela é mostrado as funções:

\begin{table}[h]
\centering
\caption{Funções básicas para utilizar a Raylib. Feito pelo autor \cite{raylib2025}}
...
o código funcionar, o que torna a linguagem com sintaxe simples, clara e concisa\cite{thinkPython, oracle2025_python}, uma imagem demonstrado abaixo:
...
o código é lido por um interpretador na máquina virtual python\cite{thinkPython}, como mostrado na imagem abaixo:
...
Todos os métodos da biblioteca são chamados pela biblioteca pygame, abaixo tem uma tabela exemplificando:
\cite{pygame2025}


\begin{table}[h]
\centering
\caption{Funções principais da biblioteca Pygame. Feito pelo autor. \cite{pygame2025}}
\begin{tabular}{|l|p{7cm}|}
\hline
...
O jogo em si focou em ser simples e abrangente para que as caracteristicas técnicas dos motores gráficos e fossem coletadas e analisádas.
...
Foi estudo comparativo de \cite{pantoja2022jogos} concluído que entre os motores \textit{Unity} 
