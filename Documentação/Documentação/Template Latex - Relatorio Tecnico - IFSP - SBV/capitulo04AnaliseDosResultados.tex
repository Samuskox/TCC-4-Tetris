...
Quando uma linha ou mais linhas na grade é completada, a função de remoção de linhas é chamada, \textit{cleanLines}, e é responsável por verificar quais linhas estão completas, removê-las do tabuleiro e atualizar a pontuação do jogador. Abaixo está implementado o código dessa função:
...
é chamada a função \textit{removeLine} para remover a linha do tabuleiro. A função \textit{removeLine} recebe o índice da linha a ser removida. Esse processo ocorre de baixo para cima. Abaixo está implementado o código da função \textit{removeLine}:
